...
// تحميل المقال بناءً على معامل URL
async function loadArticle() {
    ...
    if (!slug) {
        showError(getText('articleNotFound'));
        return;
    }
    ...
    try {
        ...
        showError(getText('errorLoadingArticle'));
    }
}
...
function displayAdvertisements() {
    ...
    if (contentAd) {
        ...
        if (articleContent) {
            ...
            adDiv.className = 'article-ad';
            adDiv.innerHTML = contentAd.ad_code;
            
            // إدراج الإعلان في منتصف المحتوى
            const paragraphs = articleContent.querySelectorAll('p');
            if (paragraphs.length > 4) {
                const middleIndex = Math.floor(paragraphs.length / 2);
                paragraphs[middleIndex].parentNode.insertBefore(adDiv, paragraphs[middleIndex].nextSibling);
            } else {
                articleContent.appendChild(adDiv);
            }
        }
    }
}
...
function displayArticle(article) {
    ...
    container.innerHTML = `
        ...
        <div class="share-buttons">
            <a href="#" onclick="shareArticle('facebook')" class="share-btn share-facebook">
                <i class="fab fa-facebook-f"></i> ${getText('facebook')}
            </a>
            <a href="#" onclick="shareArticle('twitter')" class="share-btn share-twitter">
                <i class="fab fa-twitter"></i> ${getText('twitter')}
            </a>
            <a href="#" onclick="shareArticle('whatsapp')" class="share-btn share-whatsapp">
                <i class="fab fa-whatsapp"></i> ${getText('whatsapp')}
            </a>
            <a href="#" onclick="shareArticle('telegram')" class="share-btn share-telegram">
                <i class="fab fa-telegram"></i> ${getText('telegram')}
            </a>
        </div>
        
        <div class="related-articles">
            <h3>${getText('relatedArticles')}</h3>
            <div id="relatedArticlesContainer" class="related-articles-grid">
                <div class="loading">${getText('loadingRelatedArticles')}</div>
            </div>
        </div>
    `;
...
function shareArticle(platform) {
    ...
    const text = currentArticle.description || currentArticle.title;
...
function toggleLanguage() {
    ...
    // إعادة تحميل المقال باللغة الجديدة
    loadArticle();
}
...
function updateLanguageTexts() {
    ...
    if (loadingText) {
        loadingText.textContent = getText('loadingArticle');
    }
...
function getText(key) {
    const texts = {
        ar: {
            ...
            errorLoadingArticle: 'حدث خطأ في تحميل المقال',
            articleNotFound: 'لم يتم تحديد المقال',
            favorites: 'المفضلة',
            news: 'أخبار',
            competitions: 'بطولات',
            more: 'المزيد',
            ...
            loadingRelatedArticles: 'جاري تحميل المقالات ذات الصلة...',
        ...
        en: {
            ...
            articleNotFound: 'Article not specified',
            favorites: 'Favorites',
            news: 'News',
            ...
            loadingRelatedArticles: 'Loading related articles...',
...
function searchArticles() {
    ...
    if (searchTerm.trim()) {
        window.location.href = `/?search=${encodeURIComponent(searchTerm)}&lang=${currentLanguage}`;
    }
}
...
async function loadRelatedArticles(category, currentArticleId) {
    try {
        ...
        document.getElementById('relatedArticlesContainer').innerHTML = 
            `<div class="no-related">${getText('noRelatedArticles')}</div>`;
    }
}
...
function displayRelatedArticles(articles) {
    ...
    if (articles.length === 0) {
        container.innerHTML = `<div class="no-related">${getText('noRelatedArticles')}</div>`;
        return;
    }

    container.innerHTML = articles.map(article => `
        <div class="related-article-card" onclick="openArticle('${article.slug}')">
            <div class="related-article-image">
                <img src="${article.thumbnail_url || article.image_url || 'https://via.placeholder.com/200x150/1e3a8a/ffffff?text=كسرة'}" 
                     alt="${article.title}" loading="lazy">
            </div>
            <div class="related-article-content">
                <h4 class="related-article-title">${article.title}</h4>
                <p class="related-article-date">${formatDate(article.published_at)}</p>
            </div>
        </div>
    `).join('');
}
